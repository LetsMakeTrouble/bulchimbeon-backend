...
1. **[지수 화면]** **Q2**("액세스 토큰 만료 시간이 어떻게 되나요?") 입력 → **10초 내** 🟢 **84%** + 인용(원문 스니펫) 표시 — *"영어 명세를 한국어로, 근거와 함께 즉답"*
   > ⚠️ **1단계 질문이 Q1 → Q2 로 바뀌었다** (사용자 결정 2026-08-09, 배포 세션 실측).
   > 배포 DB 실 LLM 대조에서 **Q1 은 매칭률 66 으로 🟡**, **Q2 는 84 로 🟢** 이었다.
   > 1단계는 "근거와 함께 즉답"을 보여 주는 자리이므로 🟡(확인대기)로는 서사가 성립하지 않는다.
   > 근거는 api-spec 의 `Authentication` 섹션이라 "영어 명세를 한국어로"라는 대사도 그대로 산다.
   > ⛔ **Q2 는 이력에서 승인 금지**다 (`demo_questions.NO_APPROVAL_KEYS`). 승인되면 공식 Q&A 가
   > 생겨 데모 당일 1단계가 **재사용**으로 빠지고 인용 화면이 사라진다.
   > 📌 **"84%"에 대하여**: `S`가 `s_floor`~`s_ceil` 리스케일 값이라 임베딩 모델이 바뀌면 달라진다.
   > **발표 전 리허설에서 `eval_questions.py` 로 실제 값을 다시 확인하고 대사를 맞춘다.**
   > ⏱ 실측 5.3초(Q2). `LLM_REASONING_EFFORT=minimal` 기준이며 12건 최대는 8.8초였다.
   > 예전 대사 "수 초 내"는 추론 모델 도입 전 수치라 지킬 수 없다.
...
3. `--with-history` 옵션: **질문 120~125건** (현재 123건)(**🟢 기대 질문만 35건 이상**)을 실제 파이프라인으로 실행해 이력·지표를 채운 상태로 시작 (발표 직전용)
   ...
   - 비용 가늠: 123건 × 3~4회 ≈ **400 LLM 호출** · 25~30분. 발표 당일 재실행은 피하고 전날 채워 둔다.
     ⛔ **시드도 `daily_llm_call_limit`(500)의 적용을 받는다.** `seed.py`는 파이프라인을
     **인프로세스로** 부르므로(`seed.py`가 `run_answer_pipeline` 직접 호출) 카운터가
     시드 프로세스 안에 그대로 쌓인다. 실측: 발행 93건×3회 + 🔴 30건×4회 ≈ **400회 / 500**
     = **80% 소진**. 질문을 150건 근처로 늘리면 뒷부분이 조용히 강제 🔴 `quota_exceeded`가
     되고, 시드는 "🟢 표본이 모자라니 질문셋을 늘려라"는 **엉뚱한 메시지**를 낸다.
