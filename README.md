...
## 지식이 썩지 않게 하는 규칙 네 가지

답변을 한 번 만들고 끝내면, 시간이 지나면서 **틀린 지식이 확정된 얼굴로 남습니다.**
그래서 확정 이후에도 계속 움직이는 규칙을 넣었습니다. 아래 값은 전부
`DEFAULT_SETTINGS` 에 있고 프로젝트마다 바꿀 수 있습니다.

### 참고 답변은 72시간이면 만료됩니다

`draft_expire_hours: 72` · `app/services/sweeper_service.py`

- 아무도 처리하지 않은 `draft` 답변을 스위퍼가 주기적으로 `expired` 로 내립니다
- 조건 **세 개를 전부** 만족해야 만료입니다 — 살아 있는 카드 밑의 답변은 죽이지 않습니다
- 재사용된 답변은 `expires_at` 이 `NULL` 이라 애초에 대상이 아닙니다

### "맞았다" 2건은 승인 **추천**일 뿐입니다

`RECOMMEND_APPROVE_CORRECT_COUNT = 2` · `app/services/feedback_service.py`

- 질문자 크로스체크가 2건 모이면 브리핑 최상단 `recommend_approve[]` 로 올라갑니다
- **자동 확정은 없습니다.** 확정은 끝까지 담당자만 합니다
- 크로스체크는 유저당 1건이고 번복할 수 없습니다 — 번복을 허용하면 추천이 흔들립니다
- "달랐다"는 이미 확정된 답변이라도 재검토로 되돌립니다

### 교훈은 프로젝트당 30개까지, 지운 것은 돌아오지 않습니다

`max_lessons: 30` · `app/services/lesson_service.py`

- 담당자 수정에서 뽑은 교훈은 **승인된 것만** 생성 프롬프트에 주입됩니다
- 자동 승인도 자동 삭제도 없습니다. 30개를 넘으면 정리 제안만 띄웁니다
- 삭제는 물리 삭제가 아니라 `status='deleted'` 전이입니다 — 남은 행의 `content_hash` 가
  같은 교훈의 재등록을 막습니다

### 문서가 바뀌면 그 근거를 쓴 확정 답변이 재검토로 갑니다

`app/services/review_cascade_service.py`

- 새 버전 활성화와 문서 삭제는 **같은 연쇄**를 탑니다 — 답변 입장에서는 같은 사건입니다
- 갱신은 `under_review` (해소되면 돌아옵니다), 삭제는 `archived` (돌아오지 않습니다)
- 대상은 확정 답변뿐입니다. `draft` 는 아직 지식이 아니라 72h 스위퍼에 맡깁니다
