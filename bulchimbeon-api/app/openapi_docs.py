...
def _apply_common_responses(schema: dict[str, Any]) -> None:
    ...
    for operations in schema.get("paths", {}).values():
        for method, operation in operations.items():
            ...
            # FastAPI 자동 생성 422 만 걷어낸다. 라우터가 직접 선언한 것은 스키마가 다르므로
            # 참조로 구분한다 — 자동 생성분은 HTTPValidationError 를 가리킨다.
            validation = responses.get("422")
            if isinstance(validation, dict) and "HTTPValidationError" in str(validation):
                responses.pop("422")
