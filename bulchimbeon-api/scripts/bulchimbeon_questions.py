...
HISTORY: tuple[HistoryQuestion, ...] = (
    ...
    *_family(
        ...
        "환경변수만 고치고 컨테이너를 재시작하면 프론트에 반영되나요?",
        "프론트 번들에 API 주소가 어떻게 들어가나요?",
        "API 베이스 URL 이 상대 경로면 CORS 프리플라이트가 발생하나요?",
        "빌드 타임 설정과 런타임 설정의 차이가 프론트에서 무엇인가요?",
        "Vite 환경변수를 바꾼 뒤 재빌드를 빼먹으면 어떤 증상이 나오나요?",
        "프론트가 API 요청을 보내는 기본 경로가 무엇인가요?",
    ),
    *_family(
        "Q3",
        "정적 번들 파일의 Cache-Control 을 어떻게 주나요?",
    ...
    *_family(
        ...
        "공식 Q&A 화면 경로가 어떻게 되나요?",
        "로그인 없이 접근할 수 있는 화면은 무엇인가요?",
        "브리핑 화면의 경로가 무엇인가요?",
        "멤버와 역할 관리는 어느 화면에서 하나요?",
        "없는 경로로 들어가면 어디로 이동하나요?",
        "질문을 새로 하는 화면의 경로가 무엇인가요?",
    ),
    *_family(
        "Q5",
    ...
    *_family(
        "Q8",
        "근거끼리 값이 어긋나면 답변이 어떻게 처리되나요?",
        "검색 유사도가 기준선 아래면 어떤 강제 보류가 되나요?",
        "일일 LLM 호출 상한을 넘기면 질문이 어떻게 처리되나요?",
        "방해금지 시간대에도 강제 보류는 등급을 유지하나요?",
        "모델 출력이 스키마를 못 맞추면 어떤 처리가 되나요?",
        "강제 보류 조건은 매칭률 점수와 무관하게 적용되나요?",
        "방해금지 강등 대상이 되는 보류 사유는 무엇인가요?",
    ),
    *_family(
        "Q9",
        ...
        "재사용 판정은 어떤 임베딩을 기준으로 비교하나요?",
        "재사용이 성립하면 파이프라인 나머지 단계를 계속 도나요?",
    ),
    *_family(
        "Q10",
        "브리핑 발송 시각 판정에 프로젝트 타임존 설정을 쓰나요?",
        "방해금지 시간에 도착한 확신 부족 답변은 어떻게 전달되나요?",
        "담당자를 교체하면 인박스의 카드가 사라지나요?",
        "알림 전송이 실패하면 확인 카드도 함께 사라지나요?",
        "방해금지를 꺼도 이전에 쌓인 카드가 남아 있나요?",
        "알림과 인박스 중 어느 쪽이 원본인가요?",
    ),
    *_family(
        "Q11",
    ...
    *_family(
        ...
        "업로드된 원본 파일은 어느 볼륨에 저장되나요?",
        "배포 스크립트가 시드를 자동으로 돌리나요?",
        "데이터베이스 비밀번호는 언제 적용되나요?",
