...
> top-k 안에 들어오는가**다. 청크 22개를 실제 파일에서 만들어야 그 질문이 성립한다.
>
> 임베딩만 합성한다 — `FakeLLMProvider` 의 해시 임베딩은 두 텍스트 사이 코사인이 사실상
> 0 이라(`06 §5` 한계) 그대로 두면 22개 전부 무의미한 순위가 된다. 기대 근거 청크에만
...
async def plant_seed_corpus(
    ...
    *,
    ...
    """시드 코퍼스 22청크를 문서 4개로 심는다. 돌려주는 값은 `heading_path` → `chunk_id`.
