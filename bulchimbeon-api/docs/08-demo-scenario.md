...
> ### ✅ 배포본 실측 (2026-08-09, 실 LLM · `scripts/eval_questions.py`) — **12/12 일치**
>
> | # | top-1 근거 | S | G | 매칭률 | 결과 | 기대 |
> | --- | --- | --- | --- | --- | --- | --- |
> | Q1 | `Order Lookup Response Fields` | 100 | 100 | **100** | 🟢 | 🟢 ✅ |
> | Q2 | `Authentication` | 83 | 100 | 83 | 🟢 | 🟢 ✅ |
> | Q3 | `Supported Currencies` | 94 | 100 | 94 | 🟢 | 🟢 ✅ |
> | Q4 | `Pagination for List Endpoints` | 98 | 100 | 98 | 🟢 | 🟢 ✅ |
> | Q5 | `API and Rate Limits` | 66 | 100 | 66 | 🟡 | 🟢~🟡 ✅ |
> | Q6 | `Shipping Fees` | 95 | 100 | 95 | 🟢 | 🟢 ✅ |
> | Q7 | — | 100 | — | — | 🔴 `conflict` | 🔴 ✅ |
> | Q8 | — | 37 | — | — | 🔴 `no_evidence` | 🔴 ✅ |
> | Q9 | — | 38 | — | — | 🔴 `no_evidence` | 🔴 ✅ |
> | Q11 | `Webhook Signature Verification` | 94 | 100 | 94 | 🟢 | 🟢 ✅ |
> | Q12 | `Idempotency and Key Retention` | 100 | 100 | **100** | 🟢 | 🟢 ✅ |
> | Q13 | `Sandbox` | 83 | 100 | 83 | 🟢 | 🟢 ✅ |
>
> **G 가 전부 100 이다.** 종전에는 ⑤ 가 인용 청크를 앞 500자만 보고 판정해 G 가 무너졌다
> (`09 §7.5`). 그 절단을 없애자 근거가 청크 뒤쪽에 있어도 정상 판정된다.
> 지연은 최대 15.2초(Q1), **데드라인 초과 0건**(🟢🟡 25초 · 🔴 35초).
> Q10 은 여기서 돌리지 않는다 — 이유는 `eval_questions.py` 독스트링 참조.
...
> ### 📢 대사 숫자는 **2026-08-09 재시드 실측**이다 (`09 §7.6`)
> 아래 값은 배포본에서 `eval_questions.py` 와 라이브 리허설로 실제로 확인한 것이다.
> 그래도 **발표 전 리허설에서 한 번 더 확인하고 대사를 맞춘다** — `S` 는 리스케일 값이고
> 질문 번역이 실행마다 미세하게 흔들려 한두 점은 움직인다 (`03 §4.1.3`).

1. **[지수 화면]** **Q1**("주문 조회 API 응답에 user_id 포함되나요?") 입력 → 🟢 **매칭률 100%** + 인용(원문 스니펫) 표시 — *"영어 명세를 한국어로, 근거와 함께 즉답"*
   > ⚠️ **1단계 질문은 Q1 로 되돌린다.** 2026-08-09 배포 세션에서 Q1 이 매칭률 66 🟡 로 나와
   > Q2 로 바꿨었지만, **그 이유가 사라졌다** — ⑤ 절단 버그 수정(`09 §7.5`)과 근거 섹션
   > 보강(`08 §2`) 후 재측정에서 **Q1 = S 100 · G 100 · 매칭률 100** 으로 12건 중 최고값이다.
   > Q2 도 여전히 🟢(83)이므로 예비로 쓸 수 있다.
   > ⛔ **표준 12건의 원문은 전부 이력 승인 대상에서 제외된다** (`seed.CANONICAL_TEXTS`).
   > 승인되면 공식 Q&A 가 생겨 데모 당일 1단계가 **재사용**으로 빠지고 인용 화면이 사라진다.
   > 시드가 끝날 때 12건 전부를 실제로 조회해 확인하므로 이 사고는 조용히 지나가지 않는다.
   > ⏱ 라이브 리허설 실측 **약 15초**(Q1). `LLM_REASONING_EFFORT=low` 기준이고 12건 최대도
   > 15.2초, 데드라인(🟢🟡 25초) 초과는 0건이다. **"10초 내"라고 말하지 마라** — 못 지킨다.
   > 대사는 *"20초 안에"* 또는 *"기다리는 동안 설명하면 끝나 있습니다"* 가 안전하다.
2. Q8 입력 → 🔴 보류: "근거 부족 — 담당자에게 전달했습니다" — *"모르면 지어내지 않고 정직하게 넘긴다"*
   > 리허설 실측: 상태 `held` · 사유 `no_evidence` · **질문자 화면에 답변이 아예 없다**(D11).
   > 초안은 DB 에 남아 담당자 카드에만 보인다. 약 8초.
3. **[마이크 화면(모바일 뷰)]** 확인 카드: 영어로 구조화된 배경→질문→**선택지 4개** + AI 초안 → 선택지 탭 또는 `edit`, 30초 컷 — *"노트북 없이 침대에서 검토"*
   > 리허설에서는 선택지 1번(*"Yes—apply Refund Policy v1 unchanged in Japan."*)을 탭했다.
   > 직접 쓰는 그림을 원하면 `edit` 으로 "Japan is 20 days (local law)" 를 넣어도 된다 —
   > 다음 단계의 재사용은 어느 쪽이든 똑같이 동작한다.
4. **[지수 화면]** 정정 알림(한국어) 수신 → 답변이 확정(✔)으로 갱신
   > 리허설 실측: 질문 `answered` · 답변 `verified` · 한국어 확정문 도착.
5. **[민준 화면]** Q10 입력 → 즉시 🟢 "공식 확정 답변" 재사용 — *"같은 질문에 두 번 다시 24시간이 걸리지 않는다"*
   > 리허설 실측: `source=reused` · `state=verified` · **매칭률은 `null`**(재사용이라 정상, D11) ·
   > 한국어 원문이 담당자 확정문 **그대로**다 (룰 4 — 재번역 금지).
...
6. **[지표 화면]** `GET /metrics` — 마무리 멘트

   | 지표 | 실측값 | 대사 |
   | --- | --- | --- |
   | 자동응답률 | **0.814** (목표 0.70) | "질문 10건 중 8건은 담당자를 깨우지 않고 끝납니다" |
   | 절약한 대기시간 | **2,520시간** | "근거 105건 × 24시간 가정" — **가정치라고 반드시 말한다** |
   | 🟢 실측 정확도 | **0.742** (표본 62) | |
   | 🟡 실측 정확도 | **0.837** (표본 43) | |
   | 🔴 | 표본 24 → **표본 부족** | "표본이 모자라면 숫자를 지어내지 않고 감춥니다" |
   | 재질문 즉답률 | **1.00** (재사용 6 · 미스 0) | "같은 질문은 두 번째부터 즉답입니다" |

   > 📌 `saved_wait_hours`는 `settings.saved_wait_assumption_hours`(24h) 기반 **가정치**다.
   > 등급별 정확도는 표본 30건 이상일 때만 숫자가 뜬다(D25).
   > ⚠️ **예전 대사 "🟡·🔴은 표본 부족"은 이제 틀렸다** — 🟡 는 표본 43 으로 숫자가 뜬다.
   > 표본 부족은 🔴 하나뿐이고, 그건 ⑤ 수정으로 잘못된 🔴 이 줄어든 결과다 (`09 §7.6`).
...
3. `--with-history` 옵션: **질문 120~125건** (현재 123건)(**🟢 기대 질문만 35건 이상**)을 실제 파이프라인으로 실행해 이력·지표를 채운 상태로 시작 (발표 직전용)
   ...
   > ### ⚠️ 원래 45~60건이었다 (**배포 세션 실측으로 상향, 2026-08-09**)
   > 45~60건은 "🟢 기대 질문을 35건 이상 넣으면 🟢 발행도 30건은 나온다"는 **가정**에서 나온 숫자였다.
   > 배포 DB에서 실 LLM으로 58건을 돌린 결과는 **🟢 16건**이었다 — 🟢 기대 계열조차
   > 실제로는 **27.6%만 🟢로 발행**됐고 나머지는 🟡·🔴로 흩어졌다. 패러프레이즈는 원문과
   > 다른 문장이라 매칭률이 그대로 재현되지 않기 때문이다.
   >
   > 즉 **"🟢 기대 35건"은 "🟢 발행 30건"을 보장하지 않는다.** D25의 30건은 *발행된* 🟢
   > 표본 수이므로, 발행률을 감안해 총량을 늘리는 것 외에 상한 안에서 닫히는 방법이 없다.
   > 실측 발행률 기준으로 **110건이면 🟢 30건**을 넘긴다.
   >
   > ⛔ 이 숫자를 다시 줄이려면 **먼저 `eval_questions.py`로 발행률을 재측정하라.**
   > 임베딩 방식·모델·시드 본문이 바뀌면 발행률도 함께 바뀐다.
   - 실행 후 일부 답변에 담당자 승인(`verified`)과 질문자 "맞았다"(`correct`) 피드백을 주입한다 — **`grade_accuracy`의 분자**가 여기서 나온다.
   - ⚠️ **D25의 30건은 총건수가 아니라 등급별 표본이다**(`02` 룰 10, `05 §13` 예시에서 green sample=52 `sufficient:true` / yellow sample=12 `sufficient:false`). 2026-08-09 재시드 실측은 **🟢 62 · 🟡 43 · 🔴 24** 라 🟢·🟡 은 숫자가 뜨고 **🔴 만 "표본 부족"** 이다. 표본 부족 표기 자체가 D25 가 의도한 정상 동작이므로(`02` 룰 10, `06 §7` 4겹) 6단계 대사는 "표본이 모자라면 숫자를 지어내지 않고 감춥니다"로 간다.
   - 비용 **실측**(2026-08-09, `03 §4.2`): 이력 123건 = **370 호출**(질문당 평균 2.94회),
     재질문 6건 +12, 재사용 방지 어서션 +13 → **395회**. 25~30분, API 비용 약 **$1.3**.
     발표 당일 재실행은 피하고 전날 채워 둔다.
     ⛔ **시드도 `daily_llm_call_limit`(500)의 적용을 받는다.** `seed.py`는 파이프라인을
     **인프로세스로** 부르므로(`seed.py`가 `run_answer_pipeline` 직접 호출) 카운터가
     시드 프로세스 안에 그대로 쌓인다. **395/500 = 79% 소진**이고 여유는 105회 ≈ **질문 35건**.
     이력을 160건 근처로 늘리면 뒷부분이 조용히 강제 🔴 `quota_exceeded`가 되고,
     시드는 "🟢 표본이 모자라니 질문셋을 늘려라"는 **엉뚱한 메시지**를 낸다.
     더 늘려야 하면 `projects.settings.daily_llm_call_limit`을 함께 올려라.
     (`--reset`이 프로젝트를 새로 만들므로 카운터는 실행마다 0에서 시작한다.)
     ⚠️ 시드는 API 서버와 **다른 프로세스**라 서버 쪽 카운터는 500 이 그대로 남는다 —
     발표 중 라이브 질문이 한도에 걸릴 일은 없다.
   - ⚠️ 라이브 스크립트(§4)가 던지는 **Q2·Q7·Q8·Q10 계열 전체**와, **표준 12건의 원문**(`seed.CANONICAL_TEXTS`)은 **승인(`verified`)·🔴확정 주입 대상에서 제외**한다. 승인은 공식 Q&A를 만들고(`06 §3`), 그러면 데모 당일 같은 질문이 ②에서 재사용 경로로 빠져 `matching_rate`·`search_score`·인용이 전부 `null`이 된다(`05 §6` D11, `06 §2` ②). §4 1단계의 "🟢 87% + 인용"이 화면에서 사라진다.
