...
- `scripts/seed.py` — `--with-history` 가 던지는 123건 (`08 §5` — 상한 상향 근거 포함)
...
#    `similarity_floor` 에 걸려 🔴 `no_evidence` 가 된다. "웹훅 서명 다이제스트는 어떤 문자열로
#    HMAC-SHA256 을 계산하나요" 로 고치자 0.4265 → 0.6676 이 됐다. 8건을 같은 방식으로 고쳤다.
#    ⛔ 키워드를 억지로 채우라는 뜻이 아니다 — 사람이 실제로 묻는 방식에는 주제어가 들어간다.
#
...
HISTORY: tuple[HistoryQuestion, ...] = (
    *_family(
        ...
        "모든 엔드포인트에 인증이 필요한가요?",
        "Authorization 헤더에 어떤 종류의 토큰을 넣어야 하나요?",
        "액세스 토큰을 24시간보다 오래 유지할 수 있나요?",
        "토큰은 어느 서비스가 발급하나요?",
        "토큰 갱신을 자동으로 처리할 수 있나요?",
    ),
    *_family(
        "Q4",
        ...
        "페이지 번호로 조회할 수 있나요?",
        "다음 페이지는 어떻게 요청하나요?",
        "limit 을 100보다 크게 지정할 수 있나요?",
        "목록 조회에 offset 을 쓸 수 있나요?",
        "목록 조회의 커서 페이지네이션 파라미터 이름이 무엇인가요?",
    ),
    *_family(
        "Q6",
        ...
        "반품 라벨 비용은 고객이 내나요?",
        "배송비 환불에 예외가 있나요?",
        "불량품 반품에도 반품 라벨 비용을 고객이 부담하나요?",
        "특급 배송 업그레이드 요금은 환불되나요?",
        "상품값이 전액 환불되면 배송비도 함께 환불되나요?",
        "불량품일 때 출고 배송비는 어떻게 처리되나요?",
        "선불 반품 라벨을 무료로 주는 경우가 있나요?",
        "단순 변심일 때 출고 배송비는 돌려받나요?",
        "배송비가 환불되는 유일한 경우는 무엇인가요?",
        "무료로 반품하려면 어떤 조건이어야 하나요?",
        "환불 금액에서 배송비가 차감되나요?",
        "제품 하자로 반품할 때 고객이 부담하는 비용이 있나요?",
        "배송비를 돌려받지 못하는 경우는 언제인가요?",
        "반품 사유에 따라 배송비 처리가 달라지나요?",
        "하자 상품 반품 비용을 회사가 부담하나요?",
    ),
    *_family(
        "Q11",
        "X-GlobalMart-Signature 헤더는 어떻게 확인하나요?",
        "웹훅 서명 검증에 어떤 해시를 쓰나요?",
        "웹훅 서명의 타임스탬프 허용 오차가 몇 분인가요?",
        "웹훅 리플레이 공격은 어떻게 막나요?",
        "웹훅 본문을 파싱한 뒤에 서명을 검증해도 되나요?",
        "웹훅 서명 다이제스트는 어떤 문자열로 HMAC-SHA256 을 계산하나요?",
        "웹훅 시크릿은 어디에 쓰나요?",
        "서명 헤더의 t 와 v1 은 각각 무엇인가요?",
        "HMAC 계산에 넣는 문자열 형식이 어떻게 되나요?",
        "웹훅 서명 다이제스트를 비교할 때 어떤 방식을 써야 하나요?",
        "타임스탬프가 5분을 넘으면 어떻게 처리하나요?",
        "raw body 로 검증해야 하는 이유가 무엇인가요?",
        "서명이 일치하지 않으면 어떻게 해야 하나요?",
        "웹훅 서명에 SHA256 을 사용하나요?",
        "JSON 을 파싱한 뒤 다시 직렬화해서 검증해도 되나요?",
    ),
    *_family(
        "Q13",
        "테스트 환경에서 진짜 돈이 나가나요?",
        ...
        "샌드박스 API 키 접두사가 무엇인가요?",
        "샌드박스에서 정산은 언제 이루어지나요?",
        "샌드박스 데이터가 운영으로 옮겨지나요?",
        "샌드박스에서 서명 검증을 미리 테스트할 수 있나요?",
        "샌드박스는 운영과 같은 API 버전인가요?",
        "샌드박스에서 받아 주는 카드 번호는 어디서 확인하나요?",
        "샌드박스 초기화 주기가 어떻게 되나요?",
        "sk_test_ 키로 운영 호스트를 호출할 수 있나요?",
        "샌드박스 주소가 운영과 다른가요?",
        "샌드박스에서 결제가 즉시 완료 처리되나요?",
        "샌드박스에서 아무 카드 번호나 쓸 수 있나요?",
        "샌드박스 초기화는 무슨 요일에 이루어지나요?",
        "운영 키를 샌드박스 호스트에 쓸 수 있나요?",
    ),
    ...
    *_family(
        ...
        "주문 조회 응답에 어떤 필드들이 들어 있나요?",
        "주문 status 값은 어떤 것들이 있나요?",
        "주문 응답의 total_amount 는 어떤 통화 단위로 표시되나요?",
        "user_id 는 어느 버전부터 응답에 포함됐나요?",
        "주문 응답의 currency 필드는 무엇을 나타내나요?",
        "주문 조회 응답에 order_id 가 들어 있나요?",
    ),
    *_family(
        "Q3",
        "주문에 쓸 수 있는 정산 통화가 무엇인가요?",
        "KRW 말고 어떤 통화를 쓸 수 있나요?",
        "정산 통화 코드는 어떤 표준을 따르나요?",
        "주문을 만든 뒤에 통화를 바꿀 수 있나요?",
        "USD 금액은 어떤 단위로 들어오나요?",
        "원화 금액에 소수점 단위가 있나요?",
        "한 주문에 통화를 여러 개 담을 수 있나요?",
        "플랫폼이 환전을 대신 해 주나요?",
    ),
    *_family(
        "Q5",
        "웹훅 전송이 실패하면 몇 번 재시도하나요?",
        "웹훅 재시도 간격이 어떻게 되나요?",
        "웹훅 재시도에 지수 백오프를 쓰나요?",
    ),
    *_family(
        "Q12",
        "Idempotency-Key 보관 기간이 어떻게 되나요?",
        "멱등키는 어떤 형식으로 만들어야 하나요?",
        "같은 멱등키로 다시 요청하면 어떻게 되나요?",
        "멱등키는 같은데 body 가 다르면 어떻게 되나요?",
        "GET 요청에도 멱등키가 적용되나요?",
        "멱등키는 어느 범위로 관리되나요?",
        "멱등키 보관 기간이 지나면 어떻게 되나요?",
        "멱등키를 쓰면 원래 응답 코드도 그대로 재생되나요?",
        "DELETE 요청에 멱등키를 보내면 어떻게 되나요?",
    ),
