...
3. `--with-history` 옵션: **질문 100~120건**(**🟢 기대 질문만 35건 이상**)을 실제 파이프라인으로 실행해 이력·지표를 채운 상태로 시작 (발표 직전용)
   ...
   - 비용 가늠: 110건 × 3~4회 = **330~440 LLM 호출**. 발표 당일 재실행은 피하고 전날 채워 둔다.
     ⚠️ `daily_llm_call_limit`(500)은 **API 서버 프로세스의 인메모리 카운터**라(`quota.py`)
     별도 프로세스로 도는 `seed.py`와 공유되지 않는다 — 시드가 이 한도에 걸리지는 않는다.
